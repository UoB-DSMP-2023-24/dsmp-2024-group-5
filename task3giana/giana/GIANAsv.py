...
def IdentifyVgeneCluster(sMat, thr_v):
...
def EncodeRepertoire(inputfile, outdir, exact=True, ST=3, thr_v=3.5, thr_s=3.5, VDict={},Vgene=True,thr_iso=10, gap=-6, verbose=False):
    ## No V gene version
    ...
    outfile=outdir+'/'+re.sub('\\.[txcsv]+','',outfile)+'-'+'-stackedVectorBL62.txt'
    g=open(outfile,'w')
    ...
    for kk in LDu:
        ...
        dM=np.array([EncodingCDR3sv(x[ST:-2]) for x in vss])
                    ...
                    for jj in range(ii,Nc):
                        ...
                        if (v1, v2) not in VDict:
                            if v1 == v2:
                                ...
                                sMat[jj,ii]=4
                            continue
                        sMat[ii,jj]=VDict[(v1,v2)]
                        sMat[jj,ii]=sMat[ii,jj]
                vCL=IdentifyVgeneCluster(sMat,thr_v)
        ...
        if exact:
            ...
            for cc in Cls:
                ...
                if len(cc)<=3:
                    ...
                    SSGnew=UpdateSSG(SSG, CDR3s, tmpVgenes, Vscore=VDict)
...
def ClusterCDR3(dM, flagL, thr=10, verbose=False):
...
''')
    ...
    parser.add_option("-S","--threshold_score",dest="thr_s",default=3.5, help="Threshold for Smith-Waterman alignment score.")
    parser.add_option("-G","--threshold_vgene",dest="thr_v",default=3.5,help="Threshold for variable gene comparison.")
    ...
    parser.add_option("-e","--Exact",dest="E",default=True,action="store_false",help="If False, iSMART will not perform Smith-Waterman alignment after isometric encoding.")
...
def main():
    ...
    for ff in files:
        ...
        EncodeRepertoire(ff, OutDir, ST=ST, thr_s=thr_s, thr_v=thr_v, exact=EE,VDict=VScore, Vgene=VV, thr_iso=cutoff, gap=Gap, verbose=verbose)
