    ...
    while 1:
        ...
        if flag == 0:
            ...
            for ii in range(len(vv)):
                ...
                Ig_new[ii] = (
                    sorted(list(set([idx1, idx2]))),  ## First entry records the relative index of a sequence clique
...
def EncodeRepertoireDF(alines, exact=True, ST=3, thr_v=3.7, thr_s=3.5, VDict={}, Vgene=True, thr_iso=10, gap=-6, GPU=False, verbose=False):
    ## No V gene version
    ## Encode CDR3 sequences into 96 dimensional space and perform k-means clustering
    ## If exact is True, SW alignment will be performed within each cluster after isometric encoding and clustering
...
def CalculateGianaScores(lines, cutoff, thr_s, thr_v, ST, Gap, EE, VV, GPU, verbose):
    vf = open("./giana/VgeneScores.txt")  ## Use tcrDist's Vgene 80-score calculation
    VScore = {}
    ...
    return EncodeRepertoireDF(lines, ST=ST, thr_s=thr_s, thr_v=thr_v, exact=EE, VDict=VScore, Vgene=VV, thr_iso=cutoff, gap=Gap, GPU=GPU, verbose=verbose)
...
def CommandLineParser():
    ...
    parser.add_option(
        "-d", "--directory", dest="Directory", help="Input repertoire sequencing file directory. Please make sure that all the files in the directory are input files.", default=""
    )
    parser.add_option("-f", "--file", dest="File", default="", help="Input single file of CDR3 sequences for grouping")
    parser.add_option(
        "-F", "--fileList", dest="files", default="", help="Alternative input: a file containing the full path to all the files. If given, overwrite -d and -f option"
    )
    parser.add_option(
        "-t",
        "--threshold",
    ...
    parser.add_option(
        "-T", "--startPosition", dest="ST", default=3, help="Starting position of CDR3 sequence. The first ST letters are omitted. CDR3 sequence length L must be >= ST+7 "
    )
    ...
    parser.add_option(
        "-M", "--EncodingMatrix", dest="Mat", default=False, action="store_true", help="If true, GIANA will export the isometric encoding matrix for each TCR. Default: False."
    )
